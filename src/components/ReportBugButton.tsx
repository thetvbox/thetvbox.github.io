...
export default function ReportBugButton({
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && <ReportBugPanel onClose={() => onOpenChange(false)} />}
      </AnimatePresence>
    </>
  );
}
  ...
  return (
    <Modal onClose={onClose} label="Report a bug" className="p-6">
      {status === "success" && result ? (
        <div>
          <p className="font-display text-lg font-semibold text-base-100">
            Thanks for the report
          </p>
          <p className="mt-1.5 text-sm text-base-400">
            Filed as issue #{result.number}.
          </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          ...
          <p className="text-xs text-base-600">
            Sent with the page you&apos;re on, your username, and the app
            version — no screenshot needed.
          </p>
          ...
          <div className="mt-1 flex items-center gap-3">
            <button
              type="submit"
              disabled={
                status === "saving" || !title.trim() || !description.trim()
              }
