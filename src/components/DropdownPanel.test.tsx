...
describe('DropdownPanel', () => {
  ...
  it('right-aligns to its trigger, so the panel never overflows past a right-anchored trigger', () => {
    render(
      <DropdownPanel onClose={vi.fn()} label="Notifications">
        <p>Hello</p>
      </DropdownPanel>,
    )
    expect(screen.getByRole('dialog', { name: 'Notifications' })).toHaveClass('right-0', 'origin-top-right')
  })
