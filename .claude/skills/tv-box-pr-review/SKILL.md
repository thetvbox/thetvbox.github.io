...
- Theme has a quick toggle in `Navbar.tsx` (top bar) in addition to the fuller control in Profile > Appearance -- both are expected and should read/write the same `ThemeContext`, not diverge into separate state. Glass-transparency and Report a bug stay Profile-only -- flag any change that adds a persistent top-bar/bottom-nav trigger for those two. A page's own title should use the shared `.large-title` class rather than Navbar owning a title.
