...
**Comments.** Only functions/methods -- and meaningfully-named exported constants where the name alone doesn't convey the "what" (e.g. `GLASS_SPRING` in `motion.ts`, `estimatedShowRating` in `useShowRatingsState.ts`, `VAPID_PUBLIC_KEY` in `constants.ts`) -- get a doc comment, and it's exactly one line. No paragraph comments, no inline reasoning comments, no comments on type/interface fields, no section-divider comments. `eslint-disable-next-line` directives are the one exception (they're compiler directives, not documentation, and are preserved). This was a deliberate, repo-wide decision, not an oversight -- don't reintroduce explanatory paragraph comments even for a genuinely tricky piece of logic; if it needs explaining, either the one-line doc captures the "what," or the code itself should be made clearer.
...
**Coverage bar: >80% on every metric** (statements, branch, functions, lines), with meaningful positive *and* negative flows covered, not just happy paths -- this is a hard repo-wide target, not a nice-to-have. As of the last full pass the repo sits at roughly 91% statements / 85% branch / 90% functions / 94% lines across 95 test files. When adding coverage, run `npx vitest run --coverage --coverage.reportsDirectory=/tmp/tv-box-coverage` rather than the bare `--coverage` flag -- see the next section for why. If a fix surfaces a real bug while writing its test (not just a missing test), fix the bug as part of the same pass rather than only adding a regression test around the buggy behavior.
