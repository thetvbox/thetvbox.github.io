...
- Redesigned the app icon (favicon, home-screen/PWA icon, and the in-app
  TV Box mark in the top bar/Login/passcode screen) to drop the purple
  brand-color background in favor of a neutral slate-glass gradient that
  matches the rest of the app's Liquid Glass chrome, keeping the amber
  play glyph as the one accent color. Works the same way in light and
  dark mode as before (separate light/dark source files and generated
  PNGs), and is generated by a committed script (`npm run icons`)
  instead of one-off hand-edited PNGs.
...
- The top bar's left side was completely blank on mobile once the app
  ...
  right with nothing to balance them. The wordmark (text only, no icon,
  matching how minimal the rest of the mobile chrome is) now floats on
  the left on mobile too, with the same glass-pill legibility treatment
  as the icons next to it.
- Profile's Diary/History/Watchlist/Dropped/Lists tab switcher was a
  one-off hand-rolled pill button with no haptic feedback on tap and
  slightly different sizing than every other pill-shaped control in the
  app. It now reuses the same shared `Chip` component as Search/Activity/
  History's filter chips, which brings both the haptic and the sizing in
  line with the rest of the app for free.
